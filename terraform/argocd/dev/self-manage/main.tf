...
# 見 terraform/argocd/dev/private-network/main.tf）。
...
# patch；private-network 專用資源已在 terraform/argocd/dev/private-network/
...
# 這個 root 假設 install root（terraform/argocd/dev/install，另一份 state）
